...
LIVEPROBE_PUBLIC_URL="${LIVEPROBE_PUBLIC_URL:-}"
...
if [[ -n "$CLERK_AUTHORIZED_PARTIES" ]]; then
  ...
  if [[ -n "$CLERK_PUBLISHABLE_KEY" || -n "$CLERK_FRONTEND_API_URL" ||
    -n "$LIVEPROBE_PUBLIC_URL" ]]; then
    [[ "$CLERK_PUBLISHABLE_KEY" =~ ^pk_(test|live)_[A-Za-z0-9_-]+$ ]] ||
      die "invalid CLERK_PUBLISHABLE_KEY"
    [[ "$CLERK_FRONTEND_API_URL" =~ ^https://[^/[:space:]]+$ ]] ||
      die "invalid CLERK_FRONTEND_API_URL"
    [[ "$LIVEPROBE_PUBLIC_URL" =~ ^https://[^/[:space:]]+$ ]] ||
      die "invalid LIVEPROBE_PUBLIC_URL"
  fi
elif [[ -n "$CLERK_SECRET_KEY" || -n "$CLERK_AUDIENCE" ||
